# Hey there! I'm Samiya 👋🌿

## Graduate/Junior Software Developer 👩‍💻

I'm a graduate/junior Software Developer with experience building **full-stack and mobile applications** using **C#, ASP.NET Core, Azure and Kotlin**. I believe in clean back-end systems, scalable cloud architecture and products that have a real-world impact.

I’ve delivered **cloud-hosted webites and apps that utilize RESTful APIs and payment integrations** used by real users and organisations — always with a focus on reliability, maintainability, and impact.

---

## Tech Stack </>

**Languages:** C#, Java, HTML, CSS, JavaScript, SQL, Lua, Kotlin
**Frameworks & Tools:** ASP.NET Core, MVC, .NET, REST APIs, Firebase, Firestore
**Cloud:** Microsoft Azure (Blob Storage, Queue Storage, Table Storage, File Shares)
**Architecture:** MVC, MVVM, RBAC
**Databases:** SQL Server, NoSQL, Firebase, MongoDB
**Other:** Git/GitHub, Agile/SCRUM, UI/UX

---

## Featured Projects 🌟

### PocketGarden — Android Plant Care App 🌱 

**Kotlin | MVVM | Firestore | REST APIs**
An Android app that helps users identify plants, manage a digital garden and receive intelligent care reminders.

* Offline-first architecture using Room + Firestore sync
* Real-time watering and fertilizing reminders
* Clean MVVM architecture with Coroutines and Flow

---

### I.C. Norath NGO Donation Platform 🤝

**C# | ASP.NET Core | Firebase | Zapper API | Azure**
Full-stack donation system for a charity, supporting both a website and Android app.

* Designed and built a RESTful API serving multiple clients
* Implemented role-based access control (admin, volunteer, guest)
* Integrated secure Zapper and PayFast API payment processing

---

### Deck Dreams — Cloud-Hosted E-commerce Website 🛒 

**ASP.NET Core | Azure | C#**
A retail website deployed on Microsoft Azure.

* Used Azure Blob Storage, Table Storage, Queues, and File Shares
* Focused on scalability, reliability and Object Orientated Programming.

---

### Roblox Game Development 🎮

**Lua**
Developed and published a Roblox game played by **54,000+ users worldwide**.

* Designed gameplay components and scripted interactive components
* Early exposure to user-focused development and fundemental programming concepts

---

## A Bit About Me 🌿

When I’m not coding, you’ll probably find me **outdoors, spending time with friends, playing the guitar or watching Tim Burton films**. I love building software with intention and practicality.

I’m always curious, willing to learn and excited to keep growing as a developer while working on projects that genuinely help people.

---

## Let’s Connect

* 💼 LinkedIn: [https://www.linkedin.com/in/samiya-vawda](https://www.linkedin.com/in/samiya-vawda)
* 📧 Email: [samiyavawda@gmail.com](mailto:samiyavawda@gmail.com)
